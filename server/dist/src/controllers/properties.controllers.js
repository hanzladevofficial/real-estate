...
const getProperties = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        if (favoriteIds) {
            ...
            whereCondition.push(client_1.Prisma.sql `p.id IN (${client_1.Prisma.join(favoriteIdsArray)})`);
        }
        if (priceMin) {
            whereCondition.push(client_1.Prisma.sql `p."pricePerMonth" >= ${Number(priceMin)}`);
        }
        if (priceMax) {
            whereCondition.push(client_1.Prisma.sql `p."pricePerMonth" <= ${Number(priceMax)}`);
        }
        if (beds && beds !== "any") {
            whereCondition.push(client_1.Prisma.sql `p.beds >= ${Number(beds)}`);
        }
        if (baths && baths !== "any") {
            whereCondition.push(client_1.Prisma.sql `p.baths >= ${Number(baths)}`);
        }
        if (squareFeetMin) {
            whereCondition.push(client_1.Prisma.sql `p."squareFeet" >= ${Number(squareFeetMin)}`);
        }
        if (squareFeetMax) {
            whereCondition.push(client_1.Prisma.sql `p."squareFeet" <= ${Number(squareFeetMax)}`);
        }
        if (propertyType && propertyType !== "any") {
            whereCondition.push(client_1.Prisma.sql `p."propertyType" = ${client_1.PropertyType}::"PropertyType"`);
        }
        if (amenities && amenities !== "any") {
            const amenitiesArray = amenities.split(",");
            whereCondition.push(client_1.Prisma.sql `p.amenities @> ${amenitiesArray}`);
        }
        if (availableFrom && availableFrom !== "any") {
            ...
            if (availableFromDate) {
                ...
                if (!isNaN(date.getTime())) {
                    whereCondition.push(client_1.Prisma.sql `EXISTS (
        ...
        if (latitude && longitude) {
            ...
            whereCondition.push(client_1.Prisma.sql `ST_DWithin(
      ...
      ${whereCondition.length > 0
            ? client_1.Prisma.sql `WHERE ${client_1.Prisma.join(whereCondition, " AND ")}`
...
const createProperty = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        const geocodingResponse = yield axios_1.default.get(geocodingUrl, {
            headers: {
                "User-Agent": "RealEstateApp (hanzladev.officialemail@gmail.com",
            },
        });
